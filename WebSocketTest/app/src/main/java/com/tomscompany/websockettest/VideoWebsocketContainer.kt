...
object VideoWebsocketContainer {
    ...
    fun connect() {
        ...
        val url = "ws://$serverIp:5001/ws/video"
        ...
        WebsocketContainer.sendLog(TAG, "Connecting to video endpoint: $url", "INFO")
    }
    ...
    fun sendBinary(data: ByteArray) {
        if (isConnected.get()) {
            ...
            if (success) {
                ...
                WebsocketContainer.sendLog(TAG, "Failed to send video data", "ERROR")
            }
        } else {
            Log.w(TAG, "Cannot send video data - not connected")
            WebsocketContainer.sendLog(TAG, "Cannot send video data - video socket not connected", "WARNING")
        }
    }
    ...
    private fun createWebSocketListener() = object : WebSocketListener() {
        override fun onOpen(webSocket: WebSocket, response: Response) {
            ...
            WebsocketContainer.sendLog(TAG, "Video WebSocket connected successfully", "INFO")
            reconnectAttempts = 0
        ...
        override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
            ...
            WebsocketContainer.sendLog(TAG, "Video WebSocket error: ${t.message}", "ERROR")
            t.printStackTrace()
            this@VideoWebsocketContainer.webSocket = null
            notifyConnectionStatusChanged(false)
            scheduleReconnect()
