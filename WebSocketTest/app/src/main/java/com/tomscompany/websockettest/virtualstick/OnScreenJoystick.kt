package com.tomscompany.websockettest.virtualstick
...
import com.tomscompany.websockettest.R
...
class OnScreenJoystick(context: Context?, attrs: AttributeSet) : SurfaceView(context, attrs),
    ...
    private var mGlowPaint: Paint? = null
    ...
    private fun initGraphics() {
        // Initialize HUD-style paints instead of using bitmap
        val res = context.resources
        
        // Knob paint with cyan gradient
        mKnobPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            style = Paint.Style.FILL
            shader = RadialGradient(
                0f, 0f, 1f,
                intArrayOf(
                    res.getColor(R.color.hud_primary_cyan, null),
                    res.getColor(R.color.hud_cyan_50, null),
                    res.getColor(R.color.hud_cyan_30, null)
                ),
        ...
        mBackgroundPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            ...
            strokeWidth = 2f
            color = res.getColor(R.color.hud_primary_cyan, null)
        }
    ...
    fun doDraw(pCanvas: Canvas?) {
        ...
        // Draw HUD-style joystick knob
        val knobCenterX = mKnobX + mKnobSize * 0.5f
        val knobCenterY = mKnobY + mKnobSize * 0.5f
        val knobRadius = mKnobSize * 0.5f
        
        // Draw glow effect
        mGlowPaint?.let { paint ->
            paint.shader = RadialGradient(
                knobCenterX, knobCenterY, knobRadius * 1.5f,
                intArrayOf(
                    context.resources.getColor(R.color.hud_glow_medium, null),
                    Color.TRANSPARENT
                ),
                floatArrayOf(0f, 1f),
                Shader.TileMode.CLAMP
            )
            pCanvas.drawCircle(knobCenterX, knobCenterY, knobRadius * 1.5f, paint)
        }
        
        // Draw knob with gradient
        mKnobPaint?.let { paint ->
            paint.shader = RadialGradient(
                knobCenterX, knobCenterY, knobRadius,
                intArrayOf(
                    context.resources.getColor(R.color.hud_primary_cyan, null),
                    context.resources.getColor(R.color.hud_cyan_50, null),
                    context.resources.getColor(R.color.hud_cyan_30, null)
                ),
                floatArrayOf(0f, 0.5f, 1f),
                Shader.TileMode.CLAMP
            )
            pCanvas.drawCircle(knobCenterX, knobCenterY, knobRadius * 0.8f, paint)
        }
        
        // Draw knob border
        mBackgroundPaint?.let { paint ->
            paint.style = Paint.Style.STROKE
            paint.color = context.resources.getColor(R.color.hud_primary_cyan, null)
            pCanvas.drawCircle(knobCenterX, knobCenterY, knobRadius * 0.8f, paint)
        }
    }
